...
const obtenerDetallesEcuestas = async (req, res) => {
  ...
  try {
    ...
    const ficheUsuario = await fichas.find(
      { aprendices: id },
      "_id"
    );
    ...
    const buscarEncuesta = await detalleEncuesta
      .find()
      .where("fichas")
      .equals(ficheUsuario)
      .select("-fichas -__v -_id ");
    ...
    const buscarEncuestaParaGuardarEnRespuestas = await encuesta
      ...
      .equals(encuestasID)
      .select(
        "-nombre -fechaCreado -tiempoResponder -preuntas -encuestado -activa -__v"
      );
            ...
            try {
              objeto.preguntas.forEach(async (preguntaId) => {
                ...
                const nuevaRespuesta = new Respuesta({
                  ...
                  encuesta: buscarEncuestaParaGuardarEnRespuestas,
                });
