...
const CrearPreguntasCompaneros = () => {
  ...
  const preguntasFiltradas = preguntas.filter((pregunta) => {
    ...
    const esPaginaAprendices =
      location.pathname === "/inicio-admin/Crear-preguntas-aprendices";
    const esPaginacompañeros =
      location.pathname === "/inicio-admin/Crear-preguntas-companeros";
    const espaginaJefes =
      ...
      <div className="md:flex md:justify-between mt-2">
        <p className="px-3 pr-5 font-semibold text-[19px] text-Secundario_2/[0.8]">
          <span className="font-bold">
            Asigna preguntas a una temática específica dentro del módulo en el
            que te encuentras.
          </span>
          Estas preguntas están diseñadas exclusivamente para la creación de
          encuestas relacionadas con ese módulo en particular y la temática que
          desees abordar.{" "}
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-Principal_1 to-Principal_2 font-bold ">
            Podrás acceder a estas preguntas asignadas siempre que las necesites
        ...
        <div className="rounded-md  h-[270px] overflow-y-auto mt-4 w-full md:w-570 bg-gray-200/[0.9] font-semibold text-[20px] animate-fade-down animate-duration-[1000ms]">
          {preguntasFiltradas.length > 0 ? (
            ...
            <p className=" text-2xl font-black text-center mt-6 bg-clip-text text-transparent bg-gradient-to-l from-Secundario_1 to-Secundario_2 p-9  ">
              No hay preguntas creadas aún. Seleccione una temática o cree una
              pregunta.
            </p>
          )}
        </div>
...
export default CrearPreguntasCompaneros;
